...
// No task uses the legacy (self-contained, chat-like) UI path anymore: every task renders inside
// the standard scaffold (download panel + app bar + auto initialization) and the LLM task's main
// screen is the local API server startup page.
private val allLegacyTaskIds: MutableSet<String> = mutableSetOf()
